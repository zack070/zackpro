...
# reference_calc.py (the actual correct algorithm) lives under
# tests/sealed/reference/, root-only, unreadable by the unprivileged
# stage-1 process the untrusted candidate code runs as. It must NEVER
# be imported at module load time here -- collect_agent_output.py
# (stage 1) imports this module too, and an eager import would force
# stage 1's process to read the sealed file, and any candidate code
# sharing that process could then read it too. Only score_answers
# (called exclusively from stage 2, trusted/root) imports it, lazily.
...
def score_answers(invoices, payments, jurisdiction_rules, queries, answers: Dict[str, int]) -> ScoreResult:
    ...
    sys.path.insert(0, "/tests/sealed/reference")
    import reference_calc  # lazy: stage 2 (trusted/root) only, see module docstring above
