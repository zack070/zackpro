...
# pinned installs via a dedicated venv, run against the real /app, /tests,
# /work, /logs paths. See procurement-disruption-replan/dev/local_harness.sh
# for the /usr/local/bin/python3 wrapper rationale (never symlink into a
# venv's own python3, it creates a cycle -- always a wrapper script that
# execs the real system interpreter with the venv's site-packages on
# PYTHONPATH, and always via a plain new file, never `cat >` through the
# existing symlink).
#
