...
# including a crash or timeout in either stage, must still reach the
# point where reward.txt gets written.
...
# --- Stage 2: trusted, root. Recomputes the score from the raw recorded
# mapping with no candidate code involved at all. ---
cd /tests
