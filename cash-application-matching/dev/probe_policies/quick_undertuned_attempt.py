"""Adversarial probe: a quick, minimally-tuned single annealing pass (1500
iterations, no restarts) -- simulates a first, not-yet-refined attempt
that recognizes search is needed but hasn't invested enough effort. Must
fail the bar, confirming genuine iterative refinement is required."""
import math
import random
...
def match(payments, invoices):
    ...
    for cust_payments, cust_invoices in _by_customer(payments, invoices):
        ...
        result.update(_anneal_customer(cust_payments, cust_invoices, seed_assignment, rng, iterations=1500))
    return result
